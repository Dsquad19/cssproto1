...
def serpapi_search_linkedin_profile(person: dict, api_key=None):
    ...
    logger.info(
        f"🔍 Using SerpAPI: {'Session key' if api_key else 'Environment key' if os.getenv('SERPAPI_KEY') else 'No key'}"
    )
    ...
    try:
        ...
        for res in data.get("organic_results", []):
            ...
            if not (
                full_name.lower() in title.lower()
                or full_name.lower() in snippet.lower()
            ):
        ...
        if best_result:
            ...
            logger.info(
                f"🏆 Best match {full_name}: {best_result['LinkedIn Title']} @ {best_score:.2f}"
            )
            return best_result
        else:
    ...
    try:
        ...
        for ent in entries:
            ...
            if not is_best_match(
                full_name, title, cos_th=cosine_threshold, fuzz_th=fuzzy_threshold
            ):
                continue
            cos_s = cos_sim(
                get_sentence_model().encode(full_name, convert_to_tensor=True),
...
def finalize_income_estimates(results):
    ...
    for result in results:
        ...
        if (
            result.get("Income (Estimated)")
            and result.get("Income (Estimated)") != "Unknown"
        ):
            continue
...
def enqueue_tasks(people, cosine_threshold=0.4, fuzzy_threshold=0.75, serpapi_key=None):
    ...
    for p in people:
        fut = executor.submit(
            search_person, p, cosine_threshold, fuzzy_threshold, serpapi_key
        )
...
login_layout = html.Div(
    ...
    style={"height": "100vh", "backgroundColor": "#f5f7fa", "padding": "20px"},
)
    ...
    [
        ...
        html.Div(
            [
                ...
                # Subtitle (existing)
                html.Div(
                    "Find professional profiles with precision",
                    style={
                        "fontSize": "18px",
                        "color": "#56565A",
                        "marginBottom": "30px",
            ...
            [
                html.Div(
                    [
                                    ...
                                    [
                                        ...
                                        dcc.Upload(
                                            ...
                                            children=html.Button(
                                                ...
                                                className="floating-btn upload-btn",
                                                style={
                                                    "fontSize": "18px",
                                                    "padding": "16px 36px",
                                                    "borderRadius": "16px",
                                            ...
                                            [
                                                ...
                                                html.Div(
                                                    [
                                                        ...
                                                        html.Div(
                                                            "Enter your SerpAPI key for LinkedIn profile searches",
                                                            style={
                                                                "fontSize": "14px",
                                                                "color": "#666",
                                                                "marginBottom": "15px",
                                                ...
                                                html.Div(
                                                    [
                                                        html.Label(
                                                            "Cosine Confidence Threshold (0.30 - 0.90)",
                                                            style={
                                                                "fontWeight": "500",
                                                                "fontSize": "16px",
                                                                "marginBottom": "10px",
                                                        ...
                                                        html.Div(
                                                            "Higher values require closer name matches",
                                                            style={
                                                                "fontSize": "14px",
                                                                "color": "#666",
                                                                "marginBottom": "15px",
                                                ...
                                                html.Div(
                                                    [
                                                        html.Label(
                                                            "Limit number of people to search",
                                                            style={
                                                                "fontWeight": "500",
                                                                "fontSize": "16px",
                                                                "marginBottom": "10px",
                                                ...
                                                html.Div(
                                                    [
                                                        html.Label(
                                                            "Fuzzy Matching Threshold (0.60 - 0.95)",
                                                            style={
                                                                "fontWeight": "500",
                                                                "fontSize": "16px",
                                                                "marginBottom": "10px",
                                                        ...
                                                        html.Div(
                                                            "Controls tolerance for name variations and typos",
                                                            style={
                                                                "fontSize": "14px",
                                                                "color": "#666",
                                                                "marginBottom": "15px",
                        ...
                        html.Div(
                            [
                                html.Button(
                                    ...
                                    id="search-button",
                                    className="floating-btn start-btn",
                                    style={
                        ...
                        dcc.Interval(
                            id="interval", interval=1000, n_intervals=0, disabled=False
                        ),
                    ],
                    style={
                        "width": "100%",
        ...
        html.Footer(
            [
                ...
                html.Div(
                    "Powered by Rutgers University, Newark",
                    style={"fontSize": "14px", "color": "#56565A"},
                ),
            ],
            style={
                "textAlign": "center",
...
def display_page(pathname, auth_data):
    ...
    if authenticated:
        # Return main app layout if authenticated
        return main_layout
    else:
...
app.clientside_callback(
    ...
    [Input("stop-button", "n_clicks"), Input("restart-button", "n_clicks")],
    prevent_initial_call=True,
)
...
def update_table(
    ...
    if triggered == "search-button":
        ...
        if fname and lname and university:
            person = {"First Name": fname, "Last Name": lname, "University": university}
            if grad_year:
                person["Graduation Year"] = grad_year
            people = [person]
            manual_mode_style = {
                "display": "block"
            }  # 👈 Enable the label if manually entered
