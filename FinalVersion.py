...
import flask
from flask_login
import LoginManager, UserMixin, login_user, logout_user, current_user
...
from sentence_transformers.util
import cos_sim
from rapidfuzz
import fuzz
from transformers
...
import Counter
from dash
import dcc, html, dash_table, Output, Input, State, no_update
...
# Dictionary of authorized users - you can replace with your own users# Format: username: (user_id, password_hash, display_name)
VALID_USERS = {
...
#────────────────────────────────────────────────────────────────# SerpAPI helpers— single call
for profile + inline location & income#────────────────────────────────────────────────────────────────

CITY_KEYWORDS = [#big US cities– quick heuristic list "Chicago", "Boston", "Atlanta", "Seattle", "San Francisco", "Los Angeles",
...
if not serp_key:
    logger.warning("⚠️ No SERPAPI_KEY found — falling back to Bing search.")
return None

logger.info(f "🔍 Using SerpAPI: {'Session key' if api_key else 'Environment key' if os.getenv('SERPAPI_KEY') else 'No key'}")
...
if best_result: #grab income once, based on chosen title / loc
...
def finalize_income_estimates(results):
    salary_ranges = {#
        Engineering roles
            ...
            #Business / Finance 'analyst': (75000, 120000),
            ...
            #Marketing / Sales 'marketing': (75000, 130000),
            'sales': (65000, 140000),
            'account manager': (80000, 130000),
            'customer': (60000, 100000),

            #Healthcare 'doctor': (180000, 350000),
            'physician': (200000, 400000),
            'nurse': (75000, 120000),
            'healthcare': (80000, 150000),

            #Legal 'attorney': (130000, 250000),
            'lawyer': (120000, 240000),
            'legal': (100000, 200000),

            #Education 'professor': (80000, 150000),
            'teacher': (50000, 85000),
            'educator': (55000, 90000),

            #Other common roles 'consultant': (90000, 170000),
...
#
Default range
for unknown professions
default_range = (60000, 100000)

for result in results: #Skip
if we already have an income estimate that isn 't "Unknown"
...
salary = random.randint(matched_range[0], matched_range[1])# Format with $ and commas
...
main_layout = html.Div([
    ...
    html.Div([#Top row with welcome and logout(new)
        html.Div([#Left side - Logout button
            html.Button([
            ...
            html.Div(id = "user-welcome", style = {
                'marginLeft': 'auto',
                #This pushes the element to the right 'fontSize': '16px',
                'fontWeight': '500'
            })
        ], style = {
            'display': 'flex',
    ...
    html.Div([
        html.Div([#Action buttons with improved structure
            html.Div([
                html.Div([
                    ...
                    dcc.Upload(
                        ...
                        children = html.Button(html.Span([
                            html.I(className = 'fas fa-upload', style = {
                                'marginRight': '10px'
                            }),
                            "Upload CSV"
            ...
            html.Div([
                html.Button(html.Span([
                    ...
                    'padding': '12px 24px',
                    'borderRadius': '30px',
                    #More rounded pill - style 'backgroundColor': '#20B2AA',
                    'color': '#ffffff',
                    'border': 'none',
                    ...
                    html.Div([
                        ...
                        html.Div([
                            ...
                            dcc.Slider(
                                ...
                                marks = {
                                    i: f "{i:.2f}"
                                    for i in [0.3, 0.4, 0.5, 0.6, 0.75, 0.9]
                                },
                                tooltip = {
                                    "placement": "bottom",
                                    "always_visible": True
                        ...
                        html.Div([
                            ...
                            dcc.Slider(
                                ...
                                marks = {
                                    i: f "{i:.2f}"
                                    for i in [0.6, 0.7, 0.75, 0.85, 0.95]
                                },
                                tooltip = {
                                    "placement": "bottom",
                                    "always_visible": True
            ...
            html.Div([
                html.Div([
                    html.Div([
                        ...
                        #Video player
                        html.Div([#Replace src with your actual video path
                            html.Video(
                                controls = True,
                                id = "tutorial-video",
                                src = "/assets/tutorial.mp4", #Update with your video path style = {
                                    'width': '100%',
                                    'maxHeight': '70vh',
                                    'borderRadius': '8px'
                                }
            ...
            html.Div([
                html.Button(html.Span([
                    ...
                    'fontSize': '20px',
                    #Larger font 'padding': '18px 45px',
                    #Larger padding 'borderRadius': '16px',
                    'backgroundColor': '#0072B2',
                    'color': '#ffffff',
                    'border': 'none',
                    'boxShadow': '0 8px 20px rgba(0,114,178,0.3)',
                    #More prominent shadow 'cursor': 'pointer',
                    'transition': 'all 0.3s ease',
                    'fontWeight': '600',
                    'marginTop': '10px',
                    'marginBottom': '30px',
                    'position': 'relative',
                    #For pseudo - element effects 'overflow': 'hidden'#
                    For pulse effect
                }),
            ], style = {
                'textAlign': 'center'
        ...
        html.Div(id = "progress-container", children = [
            ...
            html.Div([
                html.Button(html.Span([
                    html.I(className = 'fas fa-stop', style = {
                        'marginRight': '10px'
                    }),
                    "Stop Search"
                    ...
                    'padding': '14px 30px',
                    'borderRadius': '30px',
                    #Pill style button 'backgroundColor': '#D55E00',
                    'color': '#ffffff',
                    'border': 'none',
...
app.layout = html.Div([#Add auth - store at the top level so it 's always available
    dcc.Store(id = 'auth-store'),
...
def display_page(pathname, auth_data): #Check
if user is authenticated
authenticated = auth_data and auth_data.get('authenticated', False)

if authenticated: #Return main app layout
if authenticated
return main_layout
...
app.clientside_callback(
        ...
        ", [Output("
        stop - button ", "
        data - confirmed ", allow_duplicate = True),
...
app.index_string = app.index_string.replace('</head>', ''
        '<style>
        .floating - btn: hover {
            transform: translateY(-3 px) scale(1.02);
...
# Custom index_string with updated CSS
            for a modern and responsive design
            app.index_string = ''
            ' < !DOCTYPE html > < html >
                < head > { % metas %
                } < title > LinkedIn Search Dashboard < /title> { % favicon %
        } { % css %
        } < link rel = "stylesheet"
        href = "https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css" >
        < style >
...
#-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --#🔄Dash Callback Context#-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
            ...
            def parse_upload(contents, filename): global uploaded_people
            if not contents or not filename: return ""
            content_type,
            content_string = contents.split(',')
            decoded = base64.b64decode(content_string)
            encoding = chardet.detect(decoded)['encoding']
            try: df = pd.read_csv(io.StringIO(decoded.decode(encoding)))
            ...
            def update_table(search_clicks, interval, fname, lname, university, grad_year, cosine_val, fuzzy_val, name_limit, serpapi_data): global scraping_active,
            completed_results,
            total_tasks,
            start_scrape_time,
            final_table_ready,
            ...
            serpapi_key = None
            if serpapi_data and "api_key" in serpapi_data: serpapi_key = serpapi_data["api_key"]
            logger.info("Using SerpAPI key from advanced settings")

            if triggered == "search-button": final_table_ready = False
            finalized_table_data = []
            ...
            if fname and lname and university: person = {
                "First Name": fname,
                "Last Name": lname,
                "University": university
            }
            if grad_year: person["Graduation Year"] = grad_year
            people = [person]
            manual_mode_style = {
                "display": "block"
            }#👈
            ...
            elif uploaded_people: people = uploaded_people
            else :return no_update, no_update, no_update, "⚠️ No data provided.", no_update, True, {
                "display": "none"
            }

            if name_limit and isinstance(name_limit, int) and name_limit > 0: people = people[: name_limit]
            ...
            and scraping_active: percent = int((len(completed_results) / total_tasks) * 100)
            elapsed = time.time() - start_scrape_time
            speed = len(completed_results) / elapsed
            if elapsed > 0
            else 0
            ...
            for i,
            r in enumerate(results): r["No."] = i + 1
            ...
            if last_result_time and time.time() - last_result_time > 30: logger.warning("⏳ No progress for 30+ seconds — recommend restarting.")
            return (
                results, {
                    "width": f "{percent}%",
                    "height": "30px",
            ...
            return (
                ...
                if not final_table_ready: logger.info("📊 Finalizing income data before displaying table...")
            finalized_table_data = finalize_income_estimates(completed_results.copy())
            for i,
            r in enumerate(finalized_table_data): r["No."] = i + 1
            r.setdefault("Location (Estimated)", "Unknown")
            r.setdefault("LinkedIn URL", "")
            r.setdefault("Score", "N/A")
            ...
            def handle_stop_restart(stop_clicks, restart_clicks, stop_confirmed, restart_confirmed): global scraping_active,
            completed_results,
            total_tasks,
            pending_tasks
            ...
            and stop_confirmed: scraping_active = False
            logger.info("🛑 Search manually stopped.")

            #❌ Cancel any queued(not yet running) tasks
            for task in pending_tasks: if not task.done(): task.cancel()

            # We need to
            return default values
            for all outputs,
            but only change the search status
            for stop
            ...
            and restart_confirmed: completed_results.clear()
            total_tasks = 0
            pending_tasks.clear()
            ...
            # Return
            default values
            for all fields including advanced settings
            return ([], {
                    "width": "0%"
                },
                "0%",
                ...
                True,
                "", #first name "", #last name "", #university "", #grad year 0.4, #
                default cosine threshold 0.75, #
                default fuzzy threshold None, #
                default name limit {})# clear serpapi key
            ...
            def download_csv(n): if not completed_results: return no_update
            df = pd.DataFrame(completed_results)
            return dcc.send_data_frame(df.to_csv, "linkedin_results.csv", index = False)

            # Callback to open and close the advanced settings modal@ app.callback(
                Output("advanced-settings-modal", "style"), [Input("advanced-settings-button", "n_clicks"),
                    Input("close-advanced-settings", "n_clicks")
                ], [State("advanced-settings-modal", "style")],
                prevent_initial_call = True
            )
            def toggle_advanced_settings_modal(open_clicks, close_clicks, current_style): ctx = dash.callback_context
            triggered_id = ctx.triggered_id

            # Create a copy of the current style to modify
            new_style = dict(current_style) if current_style
            else dict()

            if triggered_id == "advanced-settings-button": new_style["display"] = "flex"#
            Show the modal
            elif triggered_id == "close-advanced-settings": new_style["display"] = "none"#
            Hide the modal

            return new_style

            # Callback to save SerpAPI key when advanced settings are closed@ app.callback(
                Output("serpapi-key-store", "data"), [Input("close-advanced-settings", "n_clicks")], [State("serpapi-key-input", "value")],
                prevent_initial_call = True
            )
            def save_serpapi_key(close_clicks, api_key): if close_clicks: if api_key: logger.info("SerpAPI key saved to session storage")
            return {
                "api_key": api_key
            }
            else: #Clear the key
            ...
            # Callback to load SerpAPI key when advanced settings are opened@ app.callback(
                Output("serpapi-key-input", "value"), [Input("advanced-settings-button", "n_clicks")], [State("serpapi-key-store", "data")],
                prevent_initial_call = True
            )
            def load_serpapi_key(open_clicks, stored_data): if open_clicks and stored_data and "api_key" in stored_data: return stored_data["api_key"]
            ...
            def update_serpapi_input(stored_data): if stored_data and "api_key" in stored_data: return stored_data["api_key"]
            return ""#
            Clear the input field
            if no key in storage

            #────────────────────────────────────────────────────────────────# Authentication Callbacks#────────────────────────────────────────────────────────────────

            # Login callback@ app.callback(
                [Output('auth-store', 'data'),
                    Output('login-error', 'children')
                ], [Input('login-button', 'n_clicks')], [State('username-input', 'value'),
                    State('password-input', 'value')
                ]
            )
            def login(n_clicks, username, password): if not n_clicks: return None, ''

                if not username or not password: return None, 'Please enter both username and password'

                if username in VALID_USERS: user_id,
            password_hash,
            name = VALID_USERS[username]
            if check_password_hash(password_hash, password): user = User(user_id, username, name)
            login_user(user)
            ...
            #
            Logout callback@ app.callback(
                Output('auth-store', 'clear_data'),
                Input('logout-button', 'n_clicks'),
                State('logout-button', 'data-confirmed')
            )
            def logout(n_clicks, confirmed): if n_clicks and confirmed: logout_user()
            return True
            return False

            # Welcome message callback@ app.callback(
                Output('user-welcome', 'children'),
                Input('auth-store', 'data')
            )
            def update_welcome(data): if data and data.get('authenticated'): return f "Welcome, {data['name']}"
            return ""

            #
            Callback to open and close the video tutorial modal@ app.callback(
                Output("video-tutorial-modal", "style"), [Input("video-tutorial-button", "n_clicks"),
                    Input("close-video-tutorial", "n_clicks")
                ], [State("video-tutorial-modal", "style")],
                prevent_initial_call = True
            )
            def toggle_video_tutorial_modal(open_clicks, close_clicks, current_style): ctx = dash.callback_context
            triggered_id = ctx.triggered_id
            ...
            if triggered_id == "video-tutorial-button": new_style["display"] = "flex"#
            Show the modal
            elif triggered_id == "close-video-tutorial": new_style["display"] = "none"#
            Hide the modal

            return new_style

            url = "http://127.0.0.1:8050/"
            try: except Exception as e: logger.warning(f "Could not open browser automatically: {e}")




            if __name__ == "__main__": port = int(os.environ.get("PORT", 8050))
